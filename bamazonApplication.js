...
var addProductQuestions = [
  ...
  },
  {
    ...
    choices: [
      {
        //Replace with function to get Dept List from DB
        name: "Books",
        value: "1"
      },
      {
...
async function updateProduct() {
  ...
  var queryResult = await queryTable(updateProductQuery, [
    updateProductResponse.stock_qty,
    updateProductResponse.unit_price,
    updateProductResponse.product_id
  ]);
...
async function repeatApplication() {
  do {
    var doYouWantToContinueResponse = await inquirerPrompt(
      doYouWantToContinueQuestions
    );
    connection.end();
